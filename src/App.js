...
function App() {
  ...
  // connect to the wallet when creating a transaction
  ...
  async function getBalance() {
    if (typeof window.ethereum != 'undefined') {
      ...
      const contract = new ethers.Contract(greeterAddress, Token.abi, account)   
  ...
  async function sendCoins() {
    if (typeof window.ethereum != 'undefined') {
      ...
      console.log(amout, " coins successfully sent to ", userAccount)     
    }
  }

  async function fetchGreeting() {
    // expect metamask extension
    if (typeof window.ethereum != 'undefined') {
      const provider = new ethers.providers.Web3Provider(window.ethereum)
